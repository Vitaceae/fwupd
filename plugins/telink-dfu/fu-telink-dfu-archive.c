				...
				GError **error)
{
	...
	for (bl_type_idx = 0; bl_type_idx < sizeof(bl_type_keys) / sizeof(bl_type_keys[0]); bl_type_idx++) {
	...
	fu_firmware_set_version_raw(FU_FIRMWARE(self), fu_telink_dfu_parse_image_version(version, bl_type_keys[bl_type_idx].ver_format));
