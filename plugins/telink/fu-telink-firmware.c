...
fu_telink_fw_get_checksum(FuFirmware *firmware, GChecksumType csum_kind, GError **error)
{
    ...
    if (!fu_firmware_has_flag(firmware, FU_FIRMWARE_FLAG_HAS_CHECKSUM)) {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED, "unable to calculate the checksum of the update binary");
        return NULL;
    }
...
static gboolean
fu_telink_fw_parse(FuFirmware *firmware, GBytes *fw, guint64 addr_start, guint64 addr_end, FwupdInstallFlags flags, GError **error)
{
    FuTelinkFw *self = FU_TELINK_FW(firmware);
    FuTelinkFwPrivate *priv = GET_PRIVATE(self);
    const guint8 *buf;
