...
struct _FuTelinkFwV1 {
    FuTelinkFwV1Class parent_instance;
};

G_DEFINE_TYPE(FuTelinkFwV1, fu_telink_fw_v1, FU_TYPE_TELINK_FW)

static GBytes *
fu_telink_fw_v1_write(FuFirmware *firmware, GError **error)
{
...
static gboolean
fu_telink_fw_v1_read_fwinfo(FuFirmware *firmware, guint8 const *buf, gsize bufsz, GError **error)
{
...
static gboolean
fu_telink_fw_v1_parse(FuFirmware *firmware, GBytes *fw, guint64 addr_start, guint64 addr_end, FwupdInstallFlags flags, GError **error)
{
    const guint8 *buf;
    gsize bufsz = 0;

    if (!FU_FIRMWARE_CLASS(fu_telink_fw_v1_parent_class)->parse(firmware, fw, addr_start, addr_end, flags, error)) {
        return FALSE;
    }
    ...
    return fu_telink_fw_v1_read_fwinfo(firmware, buf, bufsz, error);
}

static void
fu_telink_fw_v1_init(FuTelinkFwV1 *self)
{
    //todo
}

static void
fu_telink_fw_v1_class_init(FuTelinkFwV1Class *klass)
{
    FuFirmwareClass *klass_firmware = FU_FIRMWARE_CLASS(klass);
    klass_firmware->parse = fu_telink_fw_v1_parse;
    klass_firmware->write = fu_telink_fw_v1_write;
}

FuFirmware *
fu_telink_fw_v1_new(void)
{
    return FU_FIRMWARE(g_object_new(FU_TYPE_TELINK_FW_V1, NULL));
}
