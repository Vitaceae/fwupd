...
static gboolean
fu_telink_hid_dev_set_quirk_kv(FuDevice *device,
			       const gchar *key,
			       const gchar *value,
			       GError **error)
{
	...
	//    g_set_error_literal(error, FWUPD_ERROR, FWUPD_ERROR_NOT_SUPPORTED, "quirk key not
	//    supported");
	return ret;
}
...
static gboolean
fu_telink_hid_dev_dfu_sync(FuTelinkHidDev *self,
			   FuTelinkHidDevDfuInfo *dfu_info,
...
static gboolean
fu_telink_hid_dev_dfu_start(FuTelinkHidDev *self,
			    gsize img_length,
			    guint32 img_crc,
			    guint32 offset,
			    GError **error)
...
static gboolean
fu_telink_hid_dev_write_firmware_blob(FuTelinkHidDev *self,
				      GBytes *blob,
				      FuProgress *progress,
				      GError **error)
{
...
static gboolean
fu_telink_hid_dev_write_firmware(FuDevice *device,
				 FuFirmware *firmware,
				 FuProgress *progress,
				 ...
				 GError **error)
{
	...
	if (!fu_telink_hid_dev_dfu_start(self,
	...
	/* write */
	if (!fu_telink_hid_dev_write_firmware_blob(self,
						   blob,
...
fu_telink_hid_dev_init(FuTelinkHidDev *self)
{
	...
	self->modules =
	    g_ptr_array_new_with_free_func((GDestroyNotify)fu_telink_hid_dev_module_free);
	...
	fu_device_add_flag(FU_DEVICE(self),
			   FWUPD_DEVICE_FLAG_UPDATABLE | FWUPD_DEVICE_FLAG_UNSIGNED_PAYLOAD);
